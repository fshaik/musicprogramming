...
class Tempo(wx.Panel):
    def __init__(self, parent, pos, size, sf):
    ...
    def OnPaint(self, evt):
        ...
        dc.DrawRectangle(self.mposx ,self.mposy, w,10)
...
#                         startLabelH)
    ...
    def MouseUp(self, evt):
        if self.HasCapture():
            ...
            halfway = h/2
            y = self.mposy
            dist = math.fabs(y - halfway)
            change = (dist/halfway) *.25
            if(y > halfway):
                speed = 1 - change
            else:
                speed = 1 + change
